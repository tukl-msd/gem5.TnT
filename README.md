...
In [m5.c] there is a table which associates functionalities provided by **m5**
...
*M5OP_PIC* can also be defined there. It indicates the use of *Position
Independent Code* which is generated by using the code generation option
*-fPIC*. Note that the default target is in [Makefile.x86] is *m5*, but there
are others e.g. *m5op_x86.opic*. The code generation option *-fPIC* is useful
for creating code that can be dynamically linked, e.g. object files that
compose dynamic libraries **.so**.
